...
- عرض جميع السجلات داخل جدول.

- Toggle the student's status between **0** and **1**.
- تغيير حالة الطالب بين **0** و **1**.

- Automatically refresh the page after each action.
- تحديث الصفحة تلقائيًا بعد كل عملية.
...
2. Create a table named `user`.
   - أنشئ جدولًا باسم `user`.

3. Update the database connection in `index.php`.
   - حدّث بيانات الاتصال بقاعدة البيانات داخل `index.php`.

4. Upload the project to a PHP hosting service (e.g., InfinityFree).
   - ارفع المشروع إلى استضافة تدعم PHP مثل InfinityFree.

5. Open the project in your web browser.
   - افتح المشروع من خلال المتصفح.
